...
>
  Chúc mừng ngày 8/3, cục cưng của anhhh! Chúc mừng ngày 8/3, cục cưng cụa
  anhhh!
</Title>;
...
function App() {
  ...
  useEffect(() => {
    ...
    const handleUserInteraction = async () => {
      ...
      try {
        ...
        console.error("❌ Audio failed to play on user interaction", error);
      }
    };
    playAudio();
    return () => {
  ...
  const handlePlay = (e) => {
    setPlay(e.target.checked);
    e.target.checked ? musicRef.current.play() : musicRef.current.pause();
  };
  ...
  return (
    <AppContainer>
      ...
      <Title
        ...
        transition={{ duration: 1 }}
      >
        Chúc mừng ngày 8/3, cục cưng của anhhh!
      </Title>

      <PlayButton onClick={handlePlay} checked={play} ref={playRef} />
      ...
      <audio ref={musicRef} src="/bgmusic.mp3" loop={true} autoPlay={true} />
    </AppContainer>
  );
}
